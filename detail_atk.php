...
// 2. Ambil ID dari URL
$id_requested = isset($_GET['id']) ? $_GET['id'] : (isset($_GET['id_atk']) ? $_GET['id_atk'] : null);
$atk_detail = null;
...
if ($id_requested) {
    ...
    // QUERY UTAMA (Ambil detail produk yang sedang dibuka)
    $query = mysqli_query($conn, "SELECT * FROM atk WHERE id_atk = '$id_safe'");
    
    if (mysqli_num_rows($query) > 0) {
        $atk_detail = mysqli_fetch_assoc($query);
    }
}
?>
...
<head>
    ...
    <title><?php echo $atk_detail ? $atk_detail['nama_atk'] : 'Produk Tidak Ditemukan'; ?> - Jembatan Ilmu</title>
...
<body class="bg-gray-50 text-gray-800 font-sans flex flex-col min-h-screen overflow-y-scroll">
    ...
    <div class="container mx-auto px-4 py-3 flex justify-between items-center">
        ...
        <nav class="hidden md:flex space-x-8 font-semibold text-gray-700">
            ...
            <a href="atk.php" class="hover:text-primary transition duration-300 border-b-2 border-primary">ATK</a>
...
<div id="mobile-menu" class="hidden md:hidden bg-white border-t border-gray-200 px-4 py-2 text-center">
    ...
    <main class="container mx-auto px-4 py-8 flex-grow">
        ...
        <?php if (!$atk_detail): ?>
            <div class="text-center py-20">
                ...
                <p class="mb-6">Maaf, barang yang Anda cari tidak tersedia.</p>
                <a href="atk.php" class="bg-primary text-white px-6 py-2 rounded hover:bg-blue-800">Kembali ke Katalog ATK</a>
            </div>
        <?php else: ?>
            ...
            <nav class="text-sm text-gray-500 mb-6">
                ...
                <a href="atk.php" class="hover:text-primary">Alat Tulis Kantor</a> 
                <span class="mx-2">/</span> 
                <span class="text-primary font-bold"><?php echo $atk_detail['nama_atk']; ?></span>
            </nav>
                ...
                <div class="flex flex-col md:flex-row gap-10">
                    ...
                    <div class="w-full md:w-1/3 flex-shrink-0">
                        <div class="sticky top-24">
                            <img src="<?php echo $atk_detail['cover']; ?>" alt="<?php echo $atk_detail['nama_atk']; ?>" class="w-full rounded-lg shadow-md border border-gray-200 object-cover">
                        </div>
                    </div>

                    <div class="w-full md:w-2/3">
                        <span class="bg-blue-100 text-primary text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wide">
                            <?php echo $atk_detail['jenis_atk']; ?>
                        </span>
                        
                        <h1 class="text-3xl md:text-4xl font-bold text-gray-900 mt-4 mb-2">
                            <?php echo $atk_detail['nama_atk']; ?>
                        </h1>
                        <p class="text-lg text-gray-600 mb-6">
                            Merk: <span class="font-semibold text-primary"><?php echo $atk_detail['merk']; ?></span>
                        </p>

                        <div class="flex items-end mb-8 border-b border-gray-100 pb-6">
                            <span class="text-4xl font-bold text-primary">Rp <?php echo number_format($atk_detail['harga'], 0, ',', '.'); ?></span>
                            <span class="text-sm text-green-600 font-bold ml-4 mb-2"><i class="fa-solid fa-check"></i> Stok Tersedia</span>
                        </div>

                        <div class="mb-8">
                            <h3 class="text-lg font-bold text-gray-800 mb-3 border-l-4 border-accent pl-3">Deskripsi Produk</h3>
                            <p class="text-gray-600 leading-relaxed text-justify">
                                <?php echo $atk_detail['deskripsi']; ?>
                            </p>
                        </div>

                        <div class="mb-8">
                            <h3 class="text-lg font-bold text-gray-800 mb-3 border-l-4 border-accent pl-3">Spesifikasi</h3>
                            <div class="grid grid-cols-2 gap-4 text-sm">
                                <div class="p-3 bg-gray-50 rounded">
                                    <span class="block text-gray-400 text-xs">Merk / Brand</span>
                                    <span class="font-semibold"><?php echo $atk_detail['merk']; ?></span>
                                </div>
                                <div class="p-3 bg-gray-50 rounded">
                                    <span class="block text-gray-400 text-xs">Kode Barang</span>
                                    <span class="font-semibold"><?php echo $atk_detail['id_atk']; ?></span>
                                </div>
                                <div class="p-3 bg-gray-50 rounded">
                                    <span class="block text-gray-400 text-xs">Warna / Tipe</span>
                                    <span class="font-semibold"><?php echo $atk_detail['warna']; ?></span>
                                </div>
                                <div class="p-3 bg-gray-50 rounded">
                                    <span class="block text-gray-400 text-xs">Berat Est.</span>
                                    <span class="font-semibold"><?php echo $atk_detail['berat']; ?></span>
                                </div>
                            </div>
                        </div>
            ...
            <section id="atk" class="mt-1 bg-gray-50 py-12 rounded-xl">
                <div class="container mx-auto px-4">
                    
                    <div class="flex justify-between items-end mb-8">
                        <div>
                            <h2 class="text-3xl font-bold text-gray-800">Produk ATK Lainnya</h2>
                    ...
                    <?php 
                        // QUERY ATK LAIN (Random 4 biji dari database)
                        $id_safe = mysqli_real_escape_string($conn, $id_requested);
                        $query_lain = mysqli_query($conn, "SELECT * FROM atk WHERE id_atk != '$id_safe' ORDER BY RAND() LIMIT 4");
                    ...
                    ?>
                        <div class="bg-white rounded-lg shadow-sm border hover:shadow-xl transition-all duration-300 group flex flex-col h-full">
                            ...
                            <div class="p-4 flex flex-col flex-grow">
                                ...
                                <p class="text-sm text-gray-500 mb-3"><?php echo $item['merk']; ?></p>
                    ...
                    <?php 
                            ...
                            echo "<p class='col-span-4 text-center text-gray-400'>Belum ada produk ATK lain.</p>";
                        endif;
                    ?>
                    </div>
                </div>
    ...
    <script>
        function updateQty(change) {
            ...
            if(newVal < 1) newVal = 1; // Mencegah angka negatif/nol
            input.value = newVal;
            }
    </script>
