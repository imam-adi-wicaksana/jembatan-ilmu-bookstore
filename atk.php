...
if (isset($_GET['kategori'])) {
    ...
    if (!empty($filter_kategori)) {
        // Ubah array menjadi string format SQL: 'Novel','Sastra'
        // Fungsi implode menggabungkan elemen array dengan tanda koma dan kutip
        $kategori_sql = implode("','", $filter_kategori);
        
        // Masukkan ke dalam query WHERE
        // Pastikan nama kolom di database kamu benar (misal: 'kategori')
...
// Gabungkan filter harga ke query WHERE
$where .= " AND harga BETWEEN $min AND $max";

// Debugging (Opsional: Hapus baris ini kalau sudah jalan)
// echo $where; 

// Eksekusi Query
// Hasil akhirnya akan menjadi seperti: 
// SELECT * FROM buku WHERE 1=1 AND kategori IN ('Novel','Sastra') AND harga BETWEEN 0 AND 999999
$atk_tampil = query("SELECT * FROM atk $where");
?>
...
<main class="container mx-auto px-4 py-8 flex-grow">
    ...
    <div class="mb-8 border-b pb-4">
        <h1 class="text-3xl font-bold text-primary text-center">Katalog ATK</h1>
        <p class="text-gray-500 mt-1 text-center">Temukan barang favoritmu di sini</p>
    </div>
    ...
    <div class="mb-6">
        ...
        <div class="space-y-2 text-sm">
            ...
            <label class="flex items-center space-x-2 cursor-pointer hover:text-primary transition">
                <input type="checkbox" name="kategori[]" value="Paket Hemat" class="rounded text-primary focus:ring-primary" 
                <?php echo (in_array('Paket Hemat', $filter_kategori)) ? 'checked' : ''; ?>>
                <span>Paket Hemat</span>
            </label>

            <label class="flex items-center space-x-2 cursor-pointer hover:text-primary transition">
                <input type="checkbox" name="kategori[]" value="Buku Tulis" class="rounded text-primary focus:ring-primary" 
                <?php echo (in_array('Buku Tulis', $filter_kategori)) ? 'checked' : ''; ?>>
                <span>Buku Tulis</span>
            </label>

            <label class="flex items-center space-x-2 cursor-pointer hover:text-primary transition">
                <input type="checkbox" name="kategori[]" value="Pulpen" class="rounded text-primary focus:ring-primary" 
                <?php echo (in_array('Pulpen', $filter_kategori)) ? 'checked' : ''; ?>>
                <span>Pulpen</span>
            </label>

            <label class="flex items-center space-x-2 cursor-pointer hover:text-primary transition">
                <input type="checkbox" name="kategori[]" value="Elektronik" class="rounded text-primary focus:ring-primary" 
                <?php echo (in_array('Elektronik', $filter_kategori)) ? 'checked' : ''; ?>>
        ...
        <section class="w-full md:w-3/4">
            ...
            <?php else: ?>
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    <?php foreach ($atk_tampil as $item): ?>
