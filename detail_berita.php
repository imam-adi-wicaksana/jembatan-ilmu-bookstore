...
// Ambil ID dari URL
$id_request = isset($_GET['id']) ? $_GET['id'] : null;
$artikel = null;

if($id_request) {
    // Sanitasi ID untuk mencegah SQL Injection
    $id_safe = mysqli_real_escape_string($conn, $id_request);
    
    // QUERY DATABASE: Ambil 1 berita berdasarkan ID
    $query = mysqli_query($conn, "SELECT * FROM berita WHERE id = '$id_safe'");
...
<head>
    ...
    <title><?php echo $artikel ? $artikel['judul'] : 'Berita Tidak Ditemukan'; ?></title>
...
<body class="bg-gray-50 text-gray-800 font-sans flex flex-col min-h-screen overflow-y-scroll">
    ...
    <div class="container mx-auto px-4 py-3 flex justify-between items-center">
        ...
        <nav class="hidden md:flex space-x-8 font-semibold text-gray-700">
            ...
            <a href="berita.php" class="hover:text-primary transition duration-300 border-b-2 border-primary">Berita</a>
    ...
    <main class="container mx-auto px-4 py-10 flex-grow max-w-4xl">
        ...
        <?php if(!$artikel): ?>
            <div class="text-center py-20">
                ...
                <p class="text-gray-500 mb-6">Artikel yang Anda cari mungkin sudah dihapus atau tidak tersedia.</p>
                <a href="berita.php" class="bg-primary text-white px-6 py-2 rounded hover:bg-blue-800 transition">Kembali ke Berita</a>
            </div>
        <?php else: ?>
            
            <div class="text-sm text-gray-500 mb-6">
                <a href="berita.php" class="hover:text-primary"><i class="fa-solid fa-arrow-left mr-2"></i>Kembali ke Berita</a>
            </div>

            <div class="mb-8 text-center">
                <span class="bg-blue-100 text-primary px-3 py-1 rounded-full text-sm font-bold mb-4 inline-block"><?php echo $artikel['kategori']; ?></span>
                <h1 class="text-3xl md:text-5xl font-bold text-gray-900 mb-4 leading-tight"><?php echo $artikel['judul']; ?></h1>
                <div class="text-gray-500 text-sm flex justify-center gap-6">
                    <span><i class="fa-regular fa-calendar mr-2"></i><?php echo date('d M Y', strtotime($artikel['tanggal'])); ?></span>
                    <span><i class="fa-regular fa-user mr-2"></i><?php echo $artikel['penulis']; ?></span>
                </div>
            </div>

            <div class="rounded-xl overflow-hidden shadow-lg mb-10">
                <img src="<?php echo $artikel['thumbnail']; ?>" alt="Gambar Berita" 
                     class="w-full h-auto object-cover max-h-[500px]"
                     onerror="this.src='https://via.placeholder.com/800x500?text=No+Image'">
            </div>

            <div class="bg-white p-8 md:p-12 rounded-xl shadow-sm leading-relaxed text-lg text-gray-700">
                <?php echo nl2br($artikel['isi_lengkap']); ?>
            </div>

            <div class="mt-12 text-center border-t pt-8">
                <h3 class="font-bold mb-4">Bagikan artikel ini:</h3>
                <div class="flex justify-center space-x-4">
                    <button class="w-10 h-10 rounded-full bg-blue-600 text-white flex items-center justify-center hover:opacity-80"><i class="fa-brands fa-facebook-f"></i></button>
                    <button class="w-10 h-10 rounded-full bg-sky-400 text-white flex items-center justify-center hover:opacity-80"><i class="fa-brands fa-twitter"></i></button>
                    <button class="w-10 h-10 rounded-full bg-green-500 text-white flex items-center justify-center hover:opacity-80"><i class="fa-brands fa-whatsapp"></i></button>
                </div>
            </div>
